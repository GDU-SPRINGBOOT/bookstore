...
export default function UpdateBook({ onUpdate }) {
  ...
  useEffect(() => {
    const fetchBookDetails = async () => {
      try {
        ...
        setSelectedCategories(bookDetails.categories.map(categories => categories.id));
        const imageData = bookDetails.bookImage;
        if (imageData) {
  ...
  const handleSubmit = useCallback(
    ...
    [id, name, author, selectedCategories, price, description, bookImage, onUpdate]
  );
